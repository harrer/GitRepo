...
public class TicTacToe extends basicpackage.BasicBoard implements Game {
	...
	public char getLastplayer(){return letzterSpieler;}
	
	public char getNextPlayer(){
		if(letzterSpieler=='x'){return 'o';}
		else{return 'x';}
	}

	public int setze(char s, int x, int y) {
		int spieler;//int x = xx - 1, y = yy - 1, spieler;
		if (s == 'x') {spieler = 1;}
		else{spieler = 2;}
		int[][] temp = getBoard();// Durch Seiteneffekte übertragen sich Änderungen in temp auf BasicBoard.Board
		if (s == letzterSpieler) {// Falls ein Spieler zweimal hintereinander zu spielen versucht
			if (spieler == 1) {
			...
			System.out.println("Zelle " + x + "," + y + " ist belegt");
			return temp[x][y];
		} else {// Zelle ist frei
			if (spieler == 1) {
				temp[x][y] = 1;
				letzterSpieler = s;
				System.out.println("Spieler " + s + " hat auf Zelle " + x
						+ "," + y + " gesetzt");
	...
	public int fertig() {// -1 == noch nicht fertig | 0 == unentschieden | 1/2
							// Spieler x/o hat gewonnen
		int[][] b = getBoard();
		int Zx = 0, Zo = 0;// Zählen die Kreuze(x)/Kreise(o) in den Zeilen bzw.
							// Spalten
		...
		Zx = 0;
		Zo = 0;// setzt die Zähler wieder auf 0
		...
		Zx = 1;
		Zo = 1;// setzt die Zähler wieder auf zurück, diesmal auf 1
		int diagonal = b[0][0];

...
public class Board{
	...
	private int[][] board;//[y/vert][x/horizontal] 0==leer 1==schwarz 2==weiÃŸ
	//private boolean order;//false==schwarz/S1/order0, true==weiÃŸ/S2/order1
	...
	public void dreheAngrenzendeSteineUm(Move move, ArrayList<Move> richtungen){
		for (int i = 0; i < size; i++) {
			for (int j = 0; j < size; j++) {
				controller.getGui().getButtons().get(i*8+j).paintLine(null);//setzt alle Markierungen zurÃ¼ck
			}
		}
		for (int i = 0; i < richtungen.size(); i++) {
			...
			while(!(next.x<0||next.y<0||next.x>=size||next.y>=size)&&board[next.y][next.x]!=currentPlayer){
				...
				//welche Steine umgedreht worden sind, malt rote Linie darÃ¼ber.
